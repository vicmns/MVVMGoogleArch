...
abstract class CancelableNetworkBoundResource<ResultType, RequestType> @MainThread constructor(private val appExecutors: AppExecutors) {

    private val result = MediatorLiveData<Resource<ResultType>>()
    private var apiResponse: CancelableRetrofitLiveDataCall<ApiResponse<RequestType>>? = null
            ...
            if (response != null) {
                when {
                    response.isSuccessful -> appExecutors.diskIO().execute {
                        saveCallResult(response.let { processResponse(it) }!!)
    ...
    @MainThread
    protected abstract fun createCall(): CancelableRetrofitLiveDataCall<ApiResponse<RequestType>>
}
